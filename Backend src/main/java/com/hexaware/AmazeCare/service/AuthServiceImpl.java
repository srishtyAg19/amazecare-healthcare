...
public class AuthServiceImpl implements AuthService {
    ...
    public JWTAuthResponse login(LoginDto dto) {
        ...
        // Prepare user DTO with a single role as a String
        UserDTO userDto = new UserDTO();
        userDto.setId(user.getId());
        userDto.setUsername(user.getUsername());
        userDto.setEmail(user.getEmail());
        userDto.setRole(user.getRole().name()); // Convert Role enum to String
    ...
    public String register(RegisterDto dto) {
        ...
        // Set role from the provided data, if present
        if (dto.getRole() != null) {
            user.setRole(Role.valueOf(dto.getRole().toUpperCase())); // Convert String to Role enum
        }
